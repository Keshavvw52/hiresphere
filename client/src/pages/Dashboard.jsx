    ...
    HireSphere
              ...
              <div className="space-y-3">
                {navItems.map(({ path, label, icon }, i) => (
                  <NavLink
                    key={i}
                    to={`/dashboard/${path}`}
                    className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-sm font-medium  hover:scale-[1.02] ${isActive ? "bg-purple-600 text-white" : "text-gray-800 hover:bg-white/20"}`}
                  >
            ...
            <div className="bg-white/50 backdrop-blur-sm p-5 rounded-bl-3xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-semibold text-purple-700">{companyData?.name}</p>
